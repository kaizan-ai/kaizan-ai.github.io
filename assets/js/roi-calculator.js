...
(function () {
  ...
  var DEMO_URL_ABS = IS_US ? 'https://kaizan.ai/us/demo/' : 'https://kaizan.ai/demo/';
  ...
  function buildReportHTML() {
    ...
    return '<!doctype html><html lang="en"><head><meta charset="utf-8">' +
      ...
      '<div class="sub">' + esc(s.mode) + ' model · figures in ' + (IS_US ? 'USD' : 'GBP') + ' per year</div>' +
      ...
      '<div class="foot">' + (q('[data-roi="footnote"]') ? q('[data-roi="footnote"]').textContent : '') +
        ' Satisfaction is shown directionally and never ' + (IS_US ? 'monetized' : 'monetised') + '. Generated from your inputs on kaizan.ai.</div>' +
      '</div></body></html>';
  }
  ...
  function downloadBreakdown() {
    ...
    doc.text(s.mode + ' model  ·  figures in ' + (IS_US ? 'USD' : 'GBP') + ' per year', M, y); y += 18;
    ...
    var footText = ((footEl ? footEl.textContent : '') +
      ' Satisfaction is shown directionally and never ' + (IS_US ? 'monetized' : 'monetised') + '. Generated from your inputs on kaizan.ai.')
      .replace(/\s+/g, ' ').trim();
